...
**Decision**: Rule-based extraction, not a second AI/LLM call — regex for a numeric amount (Whisper reliably transcribes spoken Ukrainian numbers as digits, e.g. spoken "двісті" → transcribed "200", so no Ukrainian-number-word parsing is needed), plus a small keyword list to classify direction: expense-indicating verbs (потратив, витратив, купив, заплатив, оплатив) vs. income-indicating verbs (отримав, зарплата, дохід, повернули, продав). No amount found, or neither keyword set matches → low-confidence, triggers FR-004/FR-005's clarification path rather than a guess.
