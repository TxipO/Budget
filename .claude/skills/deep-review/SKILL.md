...
Anything whose result depends on *where* the code executes rather than *what the data says*: system timezone (`new Date(y,m,d)`, `.getDate()`/`.getMonth()` instead of UTC variants), locale (`toLocaleDateString` without a fixed locale where consistency matters), machine-specific paths, `process.cwd()` assumptions, default timeouts that differ between local Node and serverless. Ask: would this produce the same result if run in Kyiv vs UTC vs any other TZ? On Windows vs the Vercel Linux runtime? Same failure shape, different constructor: `new Date(arbitraryString)` for a non-ISO string is also environment-dependent — `"7/8/2026"` and `"08.07.2026"` parse to *different calendar days or months* depending on the runtime's locale/TZ interpretation. Found live in `import/route.ts`'s `cellDate()`, which fell back to this for any Excel cell that wasn't already a `Date` object. Fix = accept only an unambiguous format (ISO `YYYY-MM-DD`) and construct explicitly via `Date.UTC(y, m-1, d)`; anything else returns `null` (row skipped) rather than risking a silently wrong date. Any bare `new Date(someString)` where `someString` isn't guaranteed ISO-formatted is suspect for the same reason. Same failure shape again in CSS: `maxHeight: '90vh'` on a mobile modal tested clean in Chrome devtools' mobile emulation (375x812, full height, no toolbar) but left the Save button unreachable on a real Android Chrome phone with the address bar expanded — devtools reports the *layout* viewport height, not what's actually visible once real browser chrome (address bar, gesture nav) is on screen, and doesn't simulate that gap at all. Only caught via an actual phone screenshot from the user. Fix = percentage height against a `position:fixed;inset:0` ancestor instead of `vh` — fixed-position elements are sized against the real visible viewport in modern mobile browsers, sidestepping the whole `vh`/`dvh`/`svh` question. Any `vh`-based sizing on a mobile-visible modal/sheet is suspect; devtools emulation will not catch this class of bug, only a real device will.
